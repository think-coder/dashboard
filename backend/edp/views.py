...
class Edp(object):
    ...
    def teach_assess(self, request):
        ...
        if request.method == "GET":
            ...
            # 教学态度
            attitude_lst = request.POST.getlist("attitude")
            # 教学内容
            content_lst = request.POST.getlist("content")
            # 教学效果
            effect_lst = request.POST.getlist("effect")
            # 教务组织
            organization_lst = request.POST.getlist("organization")
            # 课程评价
            appraise_txt = request.POST.getlist("appraise")
            # 课程建议
            recommend_txt = request.POST.getlist("recommend")
            # 签名
            signature_txt = request.POST.getlist("signature")
